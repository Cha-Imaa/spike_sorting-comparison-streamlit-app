...
@st.cache_data
def read_info_lines(file_path):
    with open(file_path) as f:
        return f.readlines()

@st.cache_data
def get_matched_pairs(root):
    pairs = {}
    for file in root.glob("*_algo1.html"):
        base = file.stem.replace("_algo1", "")
        pairs[base] = {"algo1": file.name}
    for file in root.glob("*_algo2.html"):
        base = file.stem.replace("_algo2", "")
        if base in pairs:
            pairs[base]["algo2"] = file.name
    for file in root.glob("*_info.txt"):
        base = file.stem.replace("_info", "")
        if base in pairs:
            pairs[base]["info"] = file.name
    return pairs

# === LOAD CACHED DATA ===
summary_data = load_summary(root_dir / "global_summary.txt")
matched_pairs = get_matched_pairs(root_dir)
...
with tabs[1]:
    ...
    if matched_pairs:
        ...
        if "info" in pair_data:
            lines = read_info_lines(root_dir / pair_data["info"])
            for line in lines:
        ...
        with col1:
            ...
            st.components.v1.html(read_html(root_dir / pair_data["algo1"]), height=400)
        with col2:
            st.markdown("#### Jim's")
            st.components.v1.html(read_html(root_dir / pair_data["algo2"]), height=400)
    else:
...
with tabs[2]:
    ...
    def display_unmatched(title, folder_path, key_prefix):
        ...
        st.components.v1.html(read_html(file), height=300)
